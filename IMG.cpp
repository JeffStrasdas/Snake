...
bool LTexture::createTexture(SDL_Color color,int w, int h,SDL_Window* gWindow)
{
    if(mTexture != NULL)
        free();
    bool success = true;
...
void LTexture::renderx(int x, int y, SDL_Rect* clip, double angle, SDL_RendererFlip flip)
{
    //Set rendering space and render to screen
	SDL_Rect renderQuad = { x * scaling_factor, y * scaling_factor, mWidth, mHeight };
